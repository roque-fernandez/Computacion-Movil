...
@Component({
  selector: 'app-new-trade-chat',
  templateUrl: './new-trade-chat.page.html',
  styleUrls: ['./new-trade-chat.page.scss'],
})
export class NewTradeChatPage implements OnInit {
  ...
  otherUserSelectedBook:Book = null;
  ...
  constructor(
    ...
    this.getBookOwner();
    console.log("Otro usuario-> ",this.otherUserId);
  ...
  async uploadTrade(){
    ...
    this.trade.idBook2 = this.otherUserSelectedBook.uid;
  ...
  tradeReady(){
    if(!!this.trade.idBook1 && 
      ...
      console.log(this.trade);
      this.presentToast('Faltan campos del intercambio por elegir');
      return false;
    }
  ...
  //FUNCION QUE OBTIENE LOS LIBROS DISPONIBLES DE LOS USUARIOS DEL INTERCAMBIO
  async getAvailableBooks(){
    ...
    this.otrosLibrosSubscriber = (await this.database.getBooksByUserId(this.otherUserId)).subscribe( res => {
      if(res.length){
  ...
  printBooks(){
    if(!!this.myBooks){
      console.log("Libros propios: ");
      this.myBooks.forEach((book) => {
        console.log("Titulo: ",book.title, "  || UserId: ", book.userId, "  || BookId: ", book.uid, );
      });
    }
    if(!!this.otherUserBooks){
      console.log("Libros del otro usuario: ");
      this.otherUserBooks.forEach((book) => {
        console.log("Titulo: ",book.title, "  || UserId: ", book.userId, "  || BookId: ", book.uid, );
      });
    }
  }
